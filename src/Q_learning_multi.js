window.QMulti = (function () {
  ...
  var initLearningRate = 0.3;
  var minLearningRate = 0.3;
  var discountFactor = 0.9;
  var nonGreedyDecayDuration = 50.0;
  ...
  var _getState = function (dx, dy, dx2, dy2) {
    var dxStepSize = _.round(dx / discreteDistance);
    var dyStepSize = _.round(dy / discreteDistance);
    var dx2StepSize = _.round(dx2 / discreteDistance);
    var dy2StepSize = _.round(dy2 / discreteDistance);
    var dxDiscretized = discreteDistance * dxStepSize;
    var dyDiscretized = discreteDistance * dyStepSize;
    var dx2Discretized = discreteDistance * dx2StepSize;
    var dy2Discretized = discreteDistance * dy2StepSize;
    return [dxDiscretized, dyDiscretized, dy2Discretized];
  };
  ...
  var _getMaxQIndex = function (state) {
  ...
  var _chooseAction = function (state) {
    ...
    if (prob >= nonGreedyProb) {
      var maxIndex = _getMaxQIndex(state);
  ...
  var _learn = function (observation) {
    ...
    var nextSQMaxIndex = _getMaxQIndex(nextState);
    var nextAction = actionSpace[nextSQMaxIndex];
    var nextSQMaxValue = _getQValue(nextState, nextAction);
    var tdError = reward + (discountFactor * nextSQMaxValue) - curSAQValue;
  ...
  return function (dx, dy, dx2, dy2, hasCollision, score) {
    // Get the state
    var state = _getState(dx, dy, dx2, dy2);
