        ...
        for (j = 0; j < cellH; j++) {
          for (i = 0; i < cellW; i++) {
            ...
            this.render({
              ...
              i,
              j, -(i * this.w * 0.5) - img.width - this.w, -(j * this.h * 0.5) - img.height - this.h);
            ctx.restore();
          }
        }
      }
  ...
  var DebugMap = Ω.Map.extend({
    ...
    init: function (tileW, tileH, xTiles, yTiles, cells, walkable, seed) {
      ...
      for (var i = 0; i < numPix; i++) {
        ...
        if (i % tileW === 0 && (i / data.width | 0) % tileH === 0) {
          color = 0;
        }
        if ((i + 1) % tileW === 0 && (i / data.width | 0) % tileH === 0) {
          color = 0;
        }
        if (i % tileW === 0 && ((i / data.width | 0) + 1) % tileH === 0) {
          color = 0;
        }
        if ((i + 1) % tileW === 0 && ((i / data.width | 0) + 1) % tileH === 0) {
          color = 0;
        }
